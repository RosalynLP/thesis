...
We can investigate more explicitly by using a simple model where the experimental covariance matrix is diagonal and the theory uncertainty is fully correlated. Writing $\beta = s e$, where $s$ is the size of correlated theory uncertainty and $e^Te=1$, we have
\be
\label{eq:modelCS}
...
We can now use the posterior of $\lambda$ to calculate the autopredictions. Fist we calculate the expectation value:
\be 
...
&=[(T-D)(T-D)^T] + E[\lambda \beta (T-D)^T]\\ &\qquad + E[(T-D) \lambda \beta^T] + E[\lambda^2] \beta \beta^T.
\end{split}
\end{equation}
The first term is just $\Cov[T]$ and the last term is $\Var[\lambda] S$. To calculate the middle terms, consider
\be
\begin{split}
...
In the simple model (Eqn.~\ref{eq:modelCS}) it can be shown that
\be 
...
\begin{figure}[h]
    \begin{center}
    \makebox{\includegraphics[width=0.4\columnwidth]{correlations/plots/Xheattop.png}}
   \makebox{\includegraphics[width=0.4\columnwidth]{correlations/plots/Pheattop.png}}
    \end{center}
  \vspace{-0.55cm}

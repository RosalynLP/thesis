...
\ee
We can write the autopredictions again in the nuisance parameter formalism, so again in a slight abuse of notation
\be 
...
Now we can find the covariance of autopredictions. We start by computing the variance of $\lambda$, again using the trick of adding and subtracting $\overline{\lambda}(T^{(r)},D)$:
\be 
\begin{split}
\Var[\lambda] &= E[(\lambda - E[\lambda])^2] \\
&= E[(\lambda - \overline{\lambda}(T^{(r)},D) +  \overline{\lambda}(T^{(r)},D) - \overline{\lambda}_0)^2] \\
&= E[(\lambda - \overline{\lambda}(T^{(r)},D))^2] + E[(\overline{\lambda}(T^{(r)},D) - \overline{\lambda}_0)^2] \\
&\qquad + E[(\lambda - \overline{\lambda}(T^{(r)},D)(\overline{\lambda}(T^{(r)},D) - \overline{\lambda}_0)] \\
&\qquad +  E[(\overline{\lambda}(T^{(r)},D) - \overline{\lambda}_0)(\lambda - \overline{\lambda}(T^{(r)},D))].
\end{split}
\ee
The next step is to take the expectation value over $\lambda$, and we see that the two cross terms are 0 because $\lambda \to \overline{\lambda}$. So
\be 
\Var[\lambda] = E[(\lambda - \overline{\lambda}(T,D))^2] + \langle(\overline{\lambda}(T^{(r)},D) - \overline{\lambda}_0)^2 \rangle,
\ee
where when taking the expectation value over $T$ we use the replica average. The first term is just $Z$, and to evaluate the second term we can use Eqn.~\ref{eqn:lambdabardef} to substitute for $\overline{\lambda}$:
\be 
\Var[\lambda] = Z + \beta^T (C+S)^{-1} \langle (T^{(r)}-T_0)(T^{(r)}-T_0)^T \rangle(C+S)^{-1} \beta .
\ee
Now we can use the definitions of $Z$, Eqn.~\ref{eqn:z}, and $X$, Eqn.~\ref{eq:Xdef}, to write
\be
\begin{split}
\Var[\lambda]&= 1 - \beta^T (C+S)^{-1} \beta + \beta^T (C+S)^{-1} X (C+S)^{-1} \beta \\
&\equiv \Zbar.
\end{split}
\ee
...
\item reduces the magnitude of the fitting uncertainty by a factor $\sqrt{\sin^2 \phi + Z^2 \cos^2 \phi}$, although the resulting fitting uncertainty is still larger than it would have been had the theory uncertainty not been included in the fit. We can see this result by considering $\tilde{n} = n - \frac{s^2 \cos \phi}{s^2 + \sigma^2}e$ and calculating
\be 
\begin{split}
\sqrt{\frac{\tilde{n} \cdot \tilde{n}}{n \cdot n}} &= \sqrt{1 - \frac{2s^2 \cos^2 \phi}{s^2 + \sigma^2} + \frac{s^4 \cos^2 \phi}{(s^2+\sigma^2)^2}} \\
&= \sqrt{\sin^2 \phi + \cos^2 \phi \bigg(1 - \frac{2s^2}{s^2 + \sigma^2} + \frac{s^4}{(s^2 + \sigma^2)^2} \bigg) }\\
&= \sqrt{\sin^2 \phi + \cos^2 \phi \bigg(1 - \frac{s^2}{s^2 + \sigma^2} \bigg)^2 }\\
&= \sqrt{ \sin^2 \phi + Z^2 \cos^2 \phi},
\end{split}
\ee
where in the last line we used the fact that $Z= \frac{\sigma^2}{\sigma^2 + s^2}$.
\end{enumerate}
...
\begin{figure}[h]
    \begin{center}
    \makebox{\includegraphics[width=0.48\columnwidth, trim={1.5cm 0 0cm 0}]{correlations/plots/Xheattop.png}}
   \makebox{\includegraphics[width=0.48\columnwidth, trim={1cm 0 0 0}]{correlations/plots/Pheattop.png}}
    \end{center}
  \vspace{-0.55cm}
